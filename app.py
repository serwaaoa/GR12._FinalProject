...
model = tf.keras.models.load_model("C:\\Users\\user\\OneDrive - Ashesi University\\leadership 3\\best_model.keras")

# Load additional information from Excel
info_df = pd.read_excel("C:\\Users\\user\\OneDrive - Ashesi University\\leadership 3\\Nigerianfood_additionalinfo.xlsx")
...
def get_additional_info(Name):
    """
    Retrieve additional information from the DataFrame based on the food name.
    """
    if Name in info_df['Name'].values:
        info = info_df[info_df['Name'] == Name].iloc[0]
        return {
...
def predict_and_get_info(image):
    ...
    Name = get_food_name(predicted_class)
    additional_info = get_additional_info(Name)
...
if uploaded_file is not None:
    ...
    Name, additional_info = predict_and_get_info(image)

    # Display results
    st.write(f"Predicted Food: {Name}")
    if additional_info:
